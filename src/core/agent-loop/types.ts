...
export interface AgentInput {
  ...
  /**
   * Handoff note from the PREVIOUS rung's agent (the morph's text↔vision
   * channel). When the ladder escalates blind → hybrid → vision (or back),
   * the finishing agent's working summary — what it accomplished, what it last
   * observed, and why it handed off — is passed here so this agent CONTINUES
   * the same task instead of restarting. Generated by `summarizeForHandoff`;
